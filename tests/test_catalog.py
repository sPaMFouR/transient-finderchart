...
def test_query_gaia_dr3_falls_back_to_vizier_on_connection_error(monkeypatch):
    ...
    monkeypatch.setattr("findingchart_guiplotter.catalog.requests.post", fake_post)
    monkeypatch.setattr("findingchart_guiplotter.catalog.requests.get", fake_get)
...
def test_query_catalog_sources_dispatches_combined_catalog(monkeypatch):
    ...
    monkeypatch.setattr("findingchart_guiplotter.catalog.query_panstarrs_dr2", fake_ps1)
