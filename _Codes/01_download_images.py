...
def load_sample(csv_path: Path) -> pd.DataFrame:
    ...
    # Drop excluded/demoted entries (reclassified types, dropped from master)
    excluded = df["classification_confidence"] == "excluded"
    if excluded.any():
        print(f"  Skipping {excluded.sum()} excluded objects: "
              f"{list(df.loc[excluded, 'sn_name'])}")
...
_LS_BRICKS_TABLE_URL = f"{_LS_NERSC_BASE}/survey-bricks-dr10.fits.gz"
_LS_PIXSCALE   = 0.262    # arcsec/pixel (native LS released coadd scale)
_LS_VIEWER_MAX_PX = 512  # documented maximum for Legacy Survey viewer FITS cutouts
...
_LS_SOUTH_BANDS = {"g", "r", "i", "z"}  # DECam/DECaLS-side DR10 coadds
_LS_NORTH_BANDS = {"g", "r", "z"}       # BASS g/r + MzLS z; no north i-band
...
def _ls_resolve_brickname(ra: float, dec: float, logger: logging.Logger | None = None) -> str:
    """Resolve the official DR10 brickname using survey-bricks when available.

    Falls back to _ls_brickname() if the metadata table cannot be downloaded or
    parsed. The fallback is logged because hand-rolled brick math is acceptable
    for recovery, not something to trust silently near brick boundaries.
    """
    fallback = _ls_brickname(ra, dec)
    cache_path = CUTOUT_DIR / "ls_brick_cache" / "survey-bricks-dr10.fits.gz"

    try:
        if not cache_path.exists() or cache_path.stat().st_size < 10_000:
            import requests
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            if logger:
                logger.info(f"  LS: downloading official survey-bricks table → {cache_path}")
            resp = requests.get(_LS_BRICKS_TABLE_URL, timeout=120, stream=True)
            if resp.status_code != 200:
                if logger:
                    _ls_log_http_failure(logger, "LS", "survey-bricks table fetch", resp)
                return fallback
            _stream_to_file_with_progress(resp, cache_path, "LS_survey_bricks")

        tab = fits.getdata(str(cache_path), 1)
        names = {name.lower(): name for name in tab.names}
        required = {"brickname", "ra1", "ra2", "dec1", "dec2"}
        if not required.issubset(names):
            if logger:
                logger.warning(f"  LS: survey-bricks missing columns {required - set(names)}; using fallback brick={fallback}")
            return fallback
        ...
        dec1 = tab[names["dec1"]]
        dec2 = tab[names["dec2"]]
        normal = (ra1 <= ra2) & (ra1 <= ra_norm) & (ra_norm < ra2)
        wrapped = (ra1 > ra2) & ((ra_norm >= ra1) | (ra_norm < ra2))
        mask = (normal | wrapped) & (dec1 <= dec) & (dec < dec2)
        idx = np.where(mask)[0]
        if len(idx) == 0:
            if logger:
                logger.warning(f"  LS: official survey-bricks found no containing brick; using fallback brick={fallback}")
            return fallback

        official = str(tab[names["brickname"]][idx[0]]).strip()
        if official != fallback and logger:
            logger.warning(f"  LS: official brick={official} differs from fallback={fallback}; using official")
        return official
    except Exception as exc:
        if logger:
            logger.warning(f"  LS: survey-bricks resolution failed ({type(exc).__name__}: {exc}); using fallback brick={fallback}")
        return fallback
...
def _ls_band_available(region: str, ls_band: str) -> bool:
    """True if the requested LS filter is physically available in this region."""
    if region == "north":
        return ls_band in _LS_NORTH_BANDS
    return ls_band in _LS_SOUTH_BANDS


def _ls_provenance(region: str, ls_band: str) -> str:
    """Human-readable provenance for logs/FITS headers; not a photometric transform."""
    if region == "north":
        if ls_band in {"g", "r"}:
            return "LS-DR10 north / BASS"
        if ls_band == "z":
            return "LS-DR10 north / MzLS"
        return "LS-DR10 north / unavailable"
    return "LS-DR10 south / DECam-DECaLS"


def _ls_viewer_native_size_px(fov_arcsec: float) -> int:
    """Native-scale viewer cutout size in pixels for a square FoV."""
    return max(int(fov_arcsec / _LS_PIXSCALE) + 1, 64)


def _ls_log_http_failure(logger: logging.Logger, band: str, label: str, resp) -> None:
    """Log enough HTTP response metadata to debug archive failures later."""
    try:
        head = resp.text[:500].replace("\n", " ")
    except Exception:
        head = "<unable to read response body>"
    logger.warning(
        f"  {band}: {label} HTTP {getattr(resp, 'status_code', 'NA')} "
...
def _download_ls_brick_file(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if resp.status_code == 404:
            _ls_log_http_failure(logger, band, "NERSC brick not found", resp)
            return False
        if resp.status_code == 400:
            _ls_log_http_failure(logger, band, "NERSC no coverage/bad request", resp)
            return False
        if resp.status_code >= 500:
            if attempt < MAX_RETRIES:
                ...
                time.sleep(RETRY_WAITS[attempt - 1])
                continue
            _ls_log_http_failure(logger, band, "NERSC server failure; giving up", resp)
            return False
        if resp.status_code != 200:
            _ls_log_http_failure(logger, band, "NERSC non-200; not retrying", resp)
            return False
...
def _cutout_from_brick(
    ...
    try:
        ...
        out_hdr["LS_REGIO"] = region or ("north" if "north" in str(brick_path) else "south")
        out_hdr["LS_PROV"]  = provenance or _ls_provenance(out_hdr["LS_REGIO"], str(out_hdr.get("FILTER", band))[-1])
        out_hdr["PIXSCALE"] = _LS_PIXSCALE
...
def _download_legacysurvey_band(
    ...
    brickname = _ls_resolve_brickname(ra, dec, logger)
    brick_url = _ls_brick_url(brickname, region, ls_band)
    provenance = _ls_provenance(region, ls_band)

    logger.info(
        f"  {band}: brick={brickname}  region={region}  "
        f"filter={ls_band}  provenance={provenance}"
    )

    if not _ls_band_available(region, ls_band):
        logger.info(
            f"  {band}: {ls_band}-band is not available in LS DR10 {region}; "
            "marking no_coverage rather than fabricating a product"
        )
        update_manifest_row(manifest, sn_name, band, FLAG_NO_COVERAGE, dt=time.time() - t0)
    ...
    if brick_path.exists() and brick_path.stat().st_size > 10_000:
        try:
            with fits.open(str(brick_path), memmap=True) as _hdul:
                if len(_hdul) == 0 or all(h.data is None for h in _hdul):
                    raise OSError("cached brick has no image HDU")
            logger.info(f"  {band}: brick already cached and readable → {brick_path.name}")
        except Exception as exc:
            logger.warning(f"  {band}: cached brick is corrupt/unreadable ({type(exc).__name__}: {exc}); deleting and re-downloading")
            brick_path.unlink(missing_ok=True)
    if not (brick_path.exists() and brick_path.stat().st_size > 10_000):
        ok = _download_ls_brick_file(brick_url, brick_path, band, logger)
        if not ok:
            # ------------------------------------------------------------------
            # Fallback: LS viewer cutout API
            # ------------------------------------------------------------------
            size_px = _ls_viewer_native_size_px(fov_arcsec)
            if size_px > _LS_VIEWER_MAX_PX:
                logger.warning(
                    f"  {band}: NERSC brick download failed, and LS viewer fallback "
                    f"cannot serve native-scale {fov_arcsec:.0f} arcsec cutout: "
                    f"requested {size_px}px > max {_LS_VIEWER_MAX_PX}px. "
                    "No degraded-resolution fallback was attempted."
                )
                update_manifest_row(manifest, sn_name, band, FLAG_DOWNLOAD_FAIL,
                                    dt=time.time() - t0)
                return FLAG_DOWNLOAD_FAIL
            ...
            logger.warning(
                f"  {band}: NERSC brick download failed — falling back to LS viewer "
                f"native cutout API ({size_px}px at {_LS_PIXSCALE} arcsec/pix)"
            )
            param_attempts = [
                {
                    "ra": f"{ra:.6f}", "dec": f"{dec:.6f}",
                    "size": size_px, "pixscale": _LS_PIXSCALE,
                    "bands": ls_band, "layer": "ls-dr10",
                },
                {
                    "ra": f"{ra:.6f}", "dec": f"{dec:.6f}",
                    "width": size_px, "height": size_px, "pixscale": _LS_PIXSCALE,
                    "bands": ls_band, "layer": "ls-dr10",
                },
            ]
            resp = None
            for i, params in enumerate(param_attempts, start=1):
                try:
                    resp = requests.get(_LS_VIEWER_URL, params=params, timeout=120, stream=True)
                except Exception as exc:
                    logger.warning(f"  {band}: viewer fallback attempt {i} failed: {type(exc).__name__}: {exc}")
                    continue
                if resp.status_code == 200:
                    break
                _ls_log_http_failure(logger, band, f"viewer fallback attempt {i}", resp)
            ...
            try:
                with fits.open(str(fits_path), mode="update") as hdul:
                    hdul[0].header["SURVEY"] = "Legacy Survey DR10"
                    hdul[0].header["LS_REGIO"] = region
                    hdul[0].header["LS_PROV"] = provenance
                    hdul[0].header["PIXSCALE"] = _LS_PIXSCALE
                    hdul.flush()
            except Exception as exc:
                logger.warning(f"  {band}: could not annotate LS viewer FITS header: {exc}")
    ...
    # ------------------------------------------------------------------
    ok = _cutout_from_brick(brick_path, coord, fov_arcsec, fits_path, band, logger, region, provenance)
    dt = time.time() - t0
...
def download_one_band(
    ...
    # Important: north has BASS g/r + MzLS z; no north i-band is fabricated.
    if survey_nm == "DES":
...
def _query_hyperleda_sql(coord: SkyCoord, search_arcmin: float = 5.0, logger: logging.Logger | None = None, sn_name: str = ""):
    """
    ...
    if logger:
        logger.info(f"[D25] HyperLEDA coordinate query for {sn_name or '<unknown>'}: {url}")
    with urllib.request.urlopen(req, timeout=20) as r:
        raw = r.read().decode("utf-8", errors="replace")

    if logger and ("error" in raw.lower() or "sql" in raw.lower()[:500]):
        logger.warning(f"[D25] HyperLEDA raw response head for {sn_name}: {raw[:500]!r}")
    ...
    if not rows:
        if logger:
            logger.warning(
                f"[D25] HyperLEDA returned no parseable rows for {sn_name or '<unknown>'}; "
                f"raw_head={raw[:500]!r}"
            )
        return None
    ...
    if best is None or best_sep > search_arcmin * 60.0:
        if logger:
            logger.info(
                f"[D25] HyperLEDA rows parsed but no source within {search_arcmin:.1f}' "
                f"for {sn_name or '<unknown>'}; best_sep={best_sep:.1f} arcsec"
            )
        return None
...
def query_d25_arcsec(coord: SkyCoord, sn_name: str, host_name: str = "", logger: logging.Logger | None = None) -> float:
    """
    ...
    """
    logger = logger or logging.getLogger("fb")
    if host_name:
        logger.info(f"[D25] Host name available for {sn_name}: {host_name!r}; coordinate query remains primary")

    # ------------------------------------------------------------------
    # 1. Live HyperLEDA SQL API
    # ------------------------------------------------------------------
    try:
        row = _query_hyperleda_sql(coord, search_arcmin=5.0, logger=logger, sn_name=sn_name)
        if row is not None:
            logd25_str = row.get("logd25", "").strip()
            if logd25_str:
                logd25 = float(logd25_str)
                if np.isfinite(logd25):
                    d25 = float(10.0 ** logd25 * 6.0)
                    logger.info(
                        f"[D25] HyperLEDA match for {sn_name}: "
                        f"obj={row.get('objname', '')!r} pgc={row.get('pgc', '')!r} "
                        f"sep={float(row.get('_sep_arcsec', np.nan)):.1f} arcsec logd25={logd25:.3f} D25={d25:.1f} arcsec"
                    )
                    return d25
            else:
                logger.warning(
            ...
            logger.info(f"[D25] HyperLEDA SQL returned no usable rows within 5' of {sn_name}")
    ...
    try:
        ...
        logd25 = float(raw)
        if not np.isfinite(logd25):
            logger.warning(f"[D25] VizieR match for {sn_name} has non-finite logd25: {raw!r}")
            return None
        d25 = float(10.0 ** logd25 * 6.0)
        logger.info(
            f"[D25] VizieR VII/237 fallback match for {sn_name}: "
            f"sep={sep[best].to(u_unit.arcsec).value:.1f} arcsec logd25={logd25:.3f} D25={d25:.1f} arcsec"
        )
        return d25
    except Exception as exc:
        logger.warning(f"[D25] VizieR D25 parse failed for {sn_name}: {type(exc).__name__}: {exc}")
        return None
...
def download_one_object(
    ...
    d25 = query_d25_arcsec(coord, sn_name, host_name=host_name, logger=logger)
    if d25 is not None:
